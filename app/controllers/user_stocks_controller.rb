class UserStocksController < ApplicationController
  ...
  def create
    # If stock ID already known, link User ID and Stock ID
    if params[:stock_id].present?
      @user_stock = UserStock.new(user: current_user, stock_id: params[:stock_id])

    # Stock ID is not known, proceed to find stock ID
    else
      stock = Stock.find_by_ticker(params[:stock_ticker])
      # If the Stock has been found before and is in the db
      if stock
        @user_stock = UserStock.new(user: current_user, stock: stock)

      # Stock has never been found before, look it up add it to our db
      else
